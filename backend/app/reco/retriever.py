...
async def retrieve_relevant_items_async(
    query: str,
    db,  # AsyncSession
    user_id: int,
    limit_per_category: Optional[int] = None,
    ...
    """
    Async version of retrieve_relevant_items for use with AsyncSession.
    Retrieve relevant wardrobe items using semantic search.
    ...
    try:
        ...
        logger.error(f"Error in retrieve_relevant_items_async: {e}", exc_info=True)
        # Fallback to all items on error
        try:
            result = await db.execute(
                select(WardrobeItem).where(WardrobeItem.user_id == user_id)
